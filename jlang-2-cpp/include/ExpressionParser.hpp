    ...
    namespace parser {
        ...
        public:
            ...
            inline void prepend_tokens(std::vector<Token> new_tokens) {
                ...
                this->state->tokens.insert(this->state->tokens.begin(), new_tokens.begin(), new_tokens.end());
            }
            ...
            statements::BlockStmt *parse_block(std::function<bool(Token)> start_check, std::function<bool(Token)> end_check, std::function<bool(Token)> delim_check);
            ...
            statements::Statement *parse_binary_expr(int precedence, statements::Statement *left);
