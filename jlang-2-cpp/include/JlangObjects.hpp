...
/// TODO: Strip away unnecessary types and unify them into a generic "word"
/// type
#define ENUM_TOKENTYPE(o)                                                      \
  o(KEYWORD) /* Keywords are basically all used for statements except for      \
                the syscall keyword */                                         \
      o(INTRINSIC) o(IDENTIFIER) /* identifiers for variables and functions */ \
      o(INT_LITERAL)             /* number literals */                         \
      o(STRING_LITERAL)          /* string literals */                         \
      o(OPERATOR) /* Things that manipulate values (basically operators but    \
                     named weird) */                                           \
...
#define ENUM_OPERATOR(o)                                                       \
  o(PLUS) o(MINUS) o(MULTIPLY) o(DIVIDE) o(MODULO) o(GREATER) o(LESS) o(EQUAL) \
      o(NOT_EQUAL) o(GREATER_EQUAL) o(LESS_EQUAL)
...
#define ENUM_INTRINSIC(o)                                                      \
  o(RETURN) /* function return */                                              \
      o(SYSCALL0) o(SYSCALL1) o(SYSCALL2) o(SYSCALL3) o(SYSCALL4) o(SYSCALL5)  \
          o(PRINT) o(ADDRESS_OF) o(ALLOCATE) /* array allocation designator */ \
...
#define ENUM_EXPRTYPE(o) o(NONE) o(INTEGER) o(POINTER)
...
#define ENUM_EXPRVALTYPE(o)                                                    \
  o(EXPRESSION) o(EXPRESSION_LIST) o(INTEGER) o(STRING)

#define o(n) n,
enum class ExprValType { ENUM_EXPRVALTYPE(o) INVALID };
#undef o

#define ENUM_IDENTTYPE(o) o(VARIABLE) o(GLOBAL_VARIABLE) o(CONSTANT) o(FUNCTION)
...
#define ENUM_STATEMENT(o)                                                      \
  o(STATEMENT) o(EXPRESSION) o(BLOCK) /* Parameter block */                    \
...
#define ENUM_CONTROL(o) o(IF) o(WHILE)
#define o(n) n,
...
private:
  std::variant<int64_t, std::string, Keyword, Operator, Intrinsic, ExprType>
      value;
...
public:
  ...
  const int64_t &as_integer() const { return std::get<int64_t>(value); }
  const std::string &as_string() const { return std::get<std::string>(value); }
  const Keyword &as_keyword() const { return std::get<Keyword>(value); }
  const Operator &as_operator() const { return std::get<Operator>(value); }
  const Intrinsic &as_intrinsic() const { return std::get<Intrinsic>(value); }
  const ExprType &as_expr_type() const { return std::get<ExprType>(value); }
...
JLDEF constexpr Operator get_operator_by_name(const char *name) {
  switch (strhash::hash(name)) {
  case strhash::hash("plus"):
    return Operator::PLUS;
  case strhash::hash("minus"):
    return Operator::MINUS;
  case strhash::hash("multiply"):
    return Operator::MULTIPLY;
  case strhash::hash("divide"):
    return Operator::DIVIDE;
  case strhash::hash("modulo"):
    return Operator::MODULO;
  case strhash::hash("greater"):
    return Operator::GREATER;
  case strhash::hash("less"):
    return Operator::LESS;
  case strhash::hash("equal"):
    return Operator::EQUAL;
  case strhash::hash("not-equal"):
    return Operator::NOT_EQUAL;
  case strhash::hash("greater-equal"):
    return Operator::GREATER_EQUAL;
  case strhash::hash("less-equal"):
...
JLDEF static std::string get_token_type_value_string(TokenType type,
                                                     TokenValue value) {
  switch (type) {
...
/// \brief Contains the type, location, text and value of a Token from a parse
/// file.
typedef struct Token_s {
  ...
  Token_s(TokenType type, std::string text, Location location,
          TokenValue value) {
    this->type = type;
    this->text = text;
  ...
  std::string display() {
    return location.display() + ": " + get_enum_name(type) + "; " +
           get_token_type_value_string(type, value) + "; \"" + text + '"';
  }
} Token;
...
public:
  ...
  auto operator==(const std::string &name) const { return this->name == name; }
};
...
public:
  ...
  auto operator==(const Constant &other) const {
    bool cmp_trivial = this->name == other.name && this->type == other.type &&
                       this->size == other.size;
    if (this->type == ExprType::POINTER) {
      return cmp_trivial && std::get<std::string>(this->value) ==
...
public:
  ...
  FunProto()
      : name(""), args(std::vector<Variable>()),
        return_type(ExprType::INVALID) {}
  const std::string &get_name() const { return name; }
...
typedef struct CompilerState_s {
  ...
  const std::string dump() {
    ...
    for (auto &[name, constant] : constants) {
      ss << "bruh" << std::endl;
      ss << constant.get_name() << " : " << get_enum_name(constant.get_type())
    ...
    for (auto &[name, proto] : prototypes) {
      ...
      for (auto &arg : proto.get_args()) {
        ss << "    " << arg.get_name() << " : " << get_enum_name(arg.get_type())
           << " : " << arg.get_size() << std::endl;
      }
    }
