...
#define ENUM_OPERATOR(o)                                                       \
    o(PLUS) o(MINUS) o(MULTIPLY) o(DIVIDE) o(MODULO) o(GREATER) o(LESS)        \
        o(EQUAL) o(NOT_EQUAL) o(GREATER_EQUAL) o(LESS_EQUAL)
...
class Constant : public Variable {
  ...
  public:
    ...
    auto operator==(const Constant &other) const {
        bool cmp_trivial = this->name == other.name &&
        ...
        if (this->type == ExprType::POINTER) {
            ...
            return cmp_trivial && std::get<uint64_t>(this->value) ==
                                      std::get<uint64_t>(other.value);
        }
    }
};
    ...
    const std::string dump() {
        ...
        for (auto &[name, proto] : prototypes) {
            ...
            for (auto &arg : proto.get_args()) {
                ss << "    " << arg.get_name() << " : "
                   << get_enum_name(arg.get_type()) << " : " << arg.get_size()
                   << std::endl;
            }
        }
        ss << "------------------------\n\n";
