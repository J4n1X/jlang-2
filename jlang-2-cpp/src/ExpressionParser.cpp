...
ExpressionParser::get_ident_ref(std::string ident_name) {
  ...
  if (this->state->constants.find(ident_name) != this->state->constants.end()) {
    ...
    idents.push_back(new IdentStmt(cur_tok.location, const_.get_type(),
                                   this->state, IdentType::CONSTANT, const_));
  }
  if (this->state->prototypes.find(ident_name) !=
      this->state->prototypes.end()) {
    auto proto = this->state->prototypes[ident_name];
    idents.push_back(new IdentStmt(cur_tok.location, proto.get_return_type(),
                                   this->state, IdentType::VARIABLE, proto));
  }
  return idents;
}
...
statements::Statement *ExpressionParser::parse_int_literal_expr() {
  ...
  auto literal = new LiteralStmt(cur_tok.location, ExprType::INTEGER,
                                 this->state, this->cur_tok.value.as_integer());
  this->next_token();
  return literal;
}
...
statements::Statement *ExpressionParser::parse_intrinsic() {
  ...
  case Intrinsic::SYSCALL5: {
    ...
    return new IntrinsicStmt(prev_tok.location, ExprType::INTEGER, this->state,
                             intrinsic, args);
  }
  ...
  case Intrinsic::RETURN: {
    ...
    return new IntrinsicStmt(prev_tok.location, expr->get_type(), this->state,
                             intrinsic, expr);
  }
  case Intrinsic::DROP: {
    auto expr = parse_any();
...
FunProto &ExpressionParser::parse_fun_proto() {
  ...
  if (this->cur_tok.type != TokenType::KEYWORD ||
    ...
    throw ParserException("Expected keyword 'yields' for function prototype",
                          this->cur_tok);
  this->next_token();
  if (this->cur_tok.type != TokenType::TYPE)
    ...
    for (auto &ref : references) {
      if (get_ident_precedence(ref->get_ident_kind()) >=
        ...
        throw ParserException(
            (new std::string("Redefinition of identifier '" + cur_tok.text +
                             "', first defined at " + ref->get_loc().display()))
                ->c_str(),
            this->cur_tok);
      }
    }
...
IdentStmt *ExpressionParser::parse_variable_def() {
  ...
  if (this->state->in_scope) {
    ...
    return new IdentStmt(prev_tok.location, ExprType::NONE, this->state,
                         IdentType::VARIABLE, this->state->global_vars.at(name),
                         param);
  } else {
